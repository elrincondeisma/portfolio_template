...
class AllDaySplitter extends internal.Splitter {
    getKeyInfo() {
        return {
            allDay: {},
            timed: {},
    ...
    getKeysForEventDef(eventDef) {
        ...
        if (internal.hasBgRendering(eventDef)) {
            return ['timed', 'allDay'];
        }
        return ['allDay'];
    }
...
const DEFAULT_SLAT_LABEL_FORMAT = internal.createFormatter({
    hour: 'numeric',
    minute: '2-digit',
    omitZeroMinute: true,
    meridiem: 'short',
...
function TimeColsAxisCell(props) {
    ...
    return (preact.createElement(internal.ViewContextType.Consumer, null, (context) => {
        if (!props.isLabeled) {
        ...
        let labelFormat = // TODO: fully pre-parse
         options.slotLabelFormat == null ? DEFAULT_SLAT_LABEL_FORMAT :
            Array.isArray(options.slotLabelFormat) ? internal.createFormatter(options.slotLabelFormat[0]) :
                internal.createFormatter(options.slotLabelFormat);
        let renderProps = {
            level: 0,
            time: props.time,
        ...
        return (preact.createElement(internal.ContentContainer, { elTag: "td", elClasses: classNames, elAttrs: {
...
class TimeBodyAxis extends internal.BaseComponent {
    render() {
...
const DEFAULT_WEEK_NUM_FORMAT = internal.createFormatter({ week: 'short' });
const AUTO_ALL_DAY_MAX_EVENT_ROWS = 5;
class TimeColsView extends internal.DateComponent {
    constructor() {
        super(...arguments);
        ...
        this.renderHeadAxis = (rowKey, frameHeight = '') => {
            ...
            let dayCnt = internal.diffDays(range.start, range.end);
            // only do in day views (to avoid doing in week views that dont need it)
            let navLinkAttrs = (dayCnt === 1)
                ? internal.buildNavLinkAttrs(this.context, range.start, 'week')
                : {};
            if (options.weekNumbers && rowKey === 'day') {
                return (preact.createElement(internal.WeekNumberContainer, { elTag: "th", elClasses: [
                        'fc-timegrid-axis',
        ...
        this.renderTableRowAxis = (rowHeight) => {
            ...
            preact.createElement(internal.ContentContainer, { elTag: "td", elClasses: [
                    'fc-timegrid-axis',
                    'fc-scrollgrid-shrink',
                ], elAttrs: {
                    'aria-hidden': true,
    ...
    renderSimpleLayout(headerRowContent, allDayContent, timeContent) {
        ...
        return (preact.createElement(internal.ViewContainer, { elRef: this.rootElRef, elClasses: ['fc-timegrid'], viewSpec: context.viewSpec },
            preact.createElement(internal.SimpleScrollGrid, { liquid: !props.isHeightAuto && !props.forPrint, collapsibleWidth: props.forPrint, cols: [{ width: 'shrink' }], sections: sections })));
    }
            ...
            chunks: [
                {
                    ...
                    preact.createElement("div", { className: "fc-timegrid-axis-chunk" },
                        ...
                        preact.createElement("div", { className: "fc-timegrid-now-indicator-container" },
                            preact.createElement(internal.NowTimer, { unit: isNowIndicator ? 'minute' : 'day' /* hacky */ }, (nowDate) => {
                                let nowIndicatorTop = isNowIndicator &&
                                    slatCoords &&
                                    slatCoords.safeComputeTop(nowDate); // might return void
                                if (typeof nowIndicatorTop === 'number') {
                                    return (preact.createElement(internal.NowIndicatorContainer, { elClasses: ['fc-timegrid-now-indicator-arrow'], elStyle: { top: nowIndicatorTop }, isAxis: true, date: nowDate }));
                                }
                                return null;
                            })))),
                },
...
class TimeColsSlatsCoords {
    ...
    safeComputeTop(date) {
        ...
        if (internal.rangeContainsMarker(dateProfile.currentRange, date)) {
            let startOfDayDate = internal.startOfDay(date);
            let timeMs = date.valueOf() - startOfDayDate.valueOf();
            if (timeMs >= internal.asRoughMs(dateProfile.slotMinTime) &&
                timeMs < internal.asRoughMs(dateProfile.slotMaxTime)) {
                return this.computeTimeTop(internal.createDuration(timeMs));
            }
        }
        return null;
    }
    ...
    computeDateTop(when, startOfDayDate) {
        if (!startOfDayDate) {
            startOfDayDate = internal.startOfDay(when);
        }
        return this.computeTimeTop(internal.createDuration(when.valueOf() - startOfDayDate.valueOf()));
    }
    ...
    computeTimeTop(duration) {
        ...
        let slatCoverage = (duration.milliseconds - internal.asRoughMs(dateProfile.slotMinTime)) / internal.asRoughMs(this.slotDuration);
        let slatIndex;
        let slatRemainder;
...
class TimeColsSlatsBody extends internal.BaseComponent {
    render() {
...
*/
class TimeColsSlats extends internal.BaseComponent {
    constructor() {
        super(...arguments);
        this.rootElRef = preact.createRef();
        this.slatElRefs = new internal.RefMap();
    }
    render() {
        let { props, context } = this;
    ...
    updateSizing() {
        ...
        if (props.onCoords &&
            ...
            if (rootEl.offsetHeight) { // not hidden by css
                props.onCoords(new TimeColsSlatsCoords(new internal.PositionCache(this.rootElRef.current, collectSlatEls(this.slatElRefs.currentMap, props.slatMetas), false, true), this.props.dateProfile, context.options.slotDuration));
            }
        }
    }
}
...
class TimeColMoreLink extends internal.BaseComponent {
    render() {
        let { props } = this;
        return (preact.createElement(internal.MoreLinkContainer, { elClasses: ['fc-timegrid-more-link'], elStyle: {
                top: props.top,
                bottom: props.bottom,
...
function findNextLevelSegs(hierarchy, subjectLevel, subjectLateral) {
    ...
    for (; level < levelCnt; level += 1) {
        ...
        let entry;
        let searchIndex = internal.binarySearch(entries, subjectEntry.span.start, internal.getEntrySpanEnd);
...
function stretchWeb(topLevelNodes, totalThickness) {
    const stretchNode = cacheable((node, startCoord, prevThickness) => internal.buildEntryKey(node), (node, startCoord, prevThickness) => {
...
function webToRects(topLevelNodes) {
    let rects = [];
    const processNode = cacheable((node, levelCoord, stackDepth) => internal.buildEntryKey(node), (node, levelCoord, stackDepth) => {
...
class TimeColEvent extends internal.BaseComponent {
    render() {
        return (preact.createElement(internal.StandardEvent, Object.assign({}, this.props, { elClasses: [
                'fc-timegrid-event',
                'fc-v-event',
...
class TimeCol extends internal.BaseComponent {
    constructor() {
        super(...arguments);
    ...
    render() {
        ...
        return (preact.createElement(internal.DayCellContainer, { elTag: "td", elRef: props.elRef, elClasses: [
                'fc-timegrid-col',
            ...
            internal.hasCustomDayCellContent(options) && (preact.createElement(InnerContent, { elTag: "div", elClasses: ['fc-timegrid-col-misc'] }))))));
    }
        ...
        return (preact.createElement(preact.Fragment, null,
            ...
            segPlacements.map((segPlacement) => {
                ...
                return (preact.createElement("div", { className: 'fc-timegrid-event-harness' +
                    ...
                    preact.createElement(TimeColEvent, Object.assign({ seg: seg, isDragging: isDragging, isResizing: isResizing, isDateSelecting: isDateSelecting, isSelected: instanceId === eventSelection, isShort: isShort }, internal.getSegMeta(seg, todayRange, nowDate)))));
            })));
    }
    ...
    renderHiddenGroups(hiddenGroups, segs) {
        ...
        return (preact.createElement(preact.Fragment, null, hiddenGroups.map((hiddenGroup) => {
            ...
            return (preact.createElement(TimeColMoreLink, { key: internal.buildIsoString(internal.computeEarliestSegStart(hiddenSegs)), hiddenSegs: hiddenSegs, top: positionCss.top, bottom: positionCss.bottom, extraDateSpan: extraDateSpan, dateProfile: dateProfile, todayRange: todayRange, nowDate: nowDate, eventSelection: eventSelection, eventDrag: eventDrag, eventResize: eventResize }));
        })));
    }
    renderFillSegs(segs, fillType) {
        ...
        let children = segVCoords.map((vcoords, i) => {
            let seg = segs[i];
            return (preact.createElement("div", { key: internal.buildEventRangeKey(seg.eventRange), className: "fc-timegrid-bg-harness", style: computeSegVStyle(vcoords) }, fillType === 'bg-event' ?
                preact.createElement(internal.BgEvent, Object.assign({ seg: seg }, internal.getSegMeta(seg, props.todayRange, props.nowDate))) :
    ...
    renderNowIndicator(segs) {
        ...
        return segs.map((seg, i) => (preact.createElement(internal.NowIndicatorContainer
...
function renderPlainFgSegs(sortedFgSegs, { todayRange, nowDate, eventSelection, eventDrag, eventResize }) {
    ...
    return (preact.createElement(preact.Fragment, null, sortedFgSegs.map((seg) => {
        ...
        return (preact.createElement("div", { key: instanceId, style: { visibility: hiddenInstances[instanceId] ? 'hidden' : '' } },
            preact.createElement(TimeColEvent, Object.assign({ seg: seg, isDragging: false, isResizing: false, isDateSelecting: false, isSelected: instanceId === eventSelection, isShort: false }, internal.getSegMeta(seg, todayRange, nowDate)))));
    })));
}
...
class TimeColsContent extends internal.BaseComponent {
    constructor() {
        super(...arguments);
        this.splitFgEventSegs = internal.memoize(splitSegsByCol);
        this.splitBgEventSegs = internal.memoize(splitSegsByCol);
        this.splitBusinessHourSegs = internal.memoize(splitSegsByCol);
        this.splitNowIndicatorSegs = internal.memoize(splitSegsByCol);
        this.splitDateSelectionSegs = internal.memoize(splitSegsByCol);
        this.splitEventDrag = internal.memoize(splitInteractionByCol);
        this.splitEventResize = internal.memoize(splitInteractionByCol);
        this.rootElRef = preact.createRef();
                    ...
                    preact.createElement("tr", { role: "row" },
                        props.axis && (preact.createElement("td", { "aria-hidden": true, className: "fc-timegrid-col fc-timegrid-axis" },
                            preact.createElement("div", { className: "fc-timegrid-col-frame" },
                                preact.createElement("div", { className: "fc-timegrid-now-indicator-container" }, typeof nowIndicatorTop === 'number' && (preact.createElement(internal.NowIndicatorContainer, { elClasses: ['fc-timegrid-now-indicator-arrow'], elStyle: { top: nowIndicatorTop }, isAxis: true, date: props.nowDate })))))),
    ...
    updateCoords() {
        ...
        if (props.onColCoords &&
            ...
            props.onColCoords(new internal.PositionCache(this.rootElRef.current, collectCellEls(this.cellElRefs.currentMap, props.cells), true, // horizontal
            false));
        }
    }
}
...
----------------------------------------------------------------------------------------------------------------------*/
class TimeCols extends internal.DateComponent {
    constructor() {
        super(...arguments);
        this.processSlotOptions = internal.memoize(processSlotOptions);
        this.state = {
            slatCoords: null,
        };
    ...
    queryHit(positionLeft, positionTop) {
        ...
        if (colIndex != null && slatIndex != null) {
            ...
            let time = internal.addDurations(dateProfile.slotMinTime, internal.multiplyDuration(snapDuration, snapIndex));
...
function processSlotOptions(slotDuration, snapDurationOverride) {
    ...
    let snapsPerSlot = internal.wholeDivideDurations(slotDuration, snapDuration);
...
class DayTimeColsSlicer extends internal.Slicer {
    sliceRange(range, dayRanges) {
        let segs = [];
        for (let col = 0; col < dayRanges.length; col += 1) {
            let segRange = internal.intersectRanges(range, dayRanges[col]);
            if (segRange) {
                segs.push({
...
class DayTimeCols extends internal.DateComponent {
    constructor() {
        super(...arguments);
        this.buildDayRanges = internal.memoize(buildDayRanges);
...
function buildSlatMetas(slotMinTime, slotMaxTime, explicitLabelInterval, slotDuration, dateEnv) {
    ...
    let slatIterator = internal.createDuration(0);
    let labelInterval = explicitLabelInterval || computeLabelInterval(slotDuration);
    let metas = [];
    while (internal.asRoughMs(slatTime) < internal.asRoughMs(slotMaxTime)) {
        let date = dateEnv.add(dayStart, slatTime);
        let isLabeled = internal.wholeDivideDurations(slatIterator, labelInterval) !== null;
        metas.push({
            date,
            time: slatTime,
            key: date.toISOString(),
            isoTimeStr: internal.formatIsoTimeString(date),
            isLabeled,
        });
        slatTime = internal.addDurations(slatTime, slotDuration);
        slatIterator = internal.addDurations(slatIterator, slotDuration);
    }
    return metas;
}
...
function computeLabelInterval(slotDuration) {
    ...
    for (i = STOCK_SUB_DURATIONS.length - 1; i >= 0; i -= 1) {
        labelInterval = internal.createDuration(STOCK_SUB_DURATIONS[i]);
        slotsPerLabel = internal.wholeDivideDurations(labelInterval, slotDuration);
...
class DayTimeColsView extends TimeColsView {
    constructor() {
        super(...arguments);
        this.buildTimeColsModel = internal.memoize(buildTimeColsModel);
        this.buildSlatMetas = internal.memoize(buildSlatMetas);
    }
    render() {
        ...
        let headerContent = options.dayHeaders && (preact.createElement(internal.DayHeader, { dates: dayTableModel.headerDates, dateProfile: dateProfile, datesRepDistinctDays: true, renderIntro: hasAttachedAxis ? this.renderHeadAxis : null }));
        let allDayContent = (options.allDaySlot !== false) && ((contentArg) => (preact.createElement(internal$1.DayTable, Object.assign({}, splitProps.allDay, { dateProfile: dateProfile, dayTableModel: dayTableModel, nextDayThreshold: options.nextDayThreshold, tableMinWidth: contentArg.tableMinWidth, colGroupNode: contentArg.tableColGroupNode, renderRowIntro: hasAttachedAxis ? this.renderTableRowAxis : null, showWeekNumbers: false, expandRows: false, headerAlignElRef: this.headerElRef, clientWidth: contentArg.clientWidth, clientHeight: contentArg.clientHeight, forPrint: props.forPrint }, this.getAllDayMaxEventProps()))));
...
function buildTimeColsModel(dateProfile, dateProfileGenerator) {
    ...
    return new internal.DayTableModel(daySeries, false);
}
